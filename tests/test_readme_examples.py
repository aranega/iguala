from ast import Assign, ClassDef, FunctionDef, Module, arg, parse
...
def test_match_direct_cls_extract_name2():
    pattern = match(Module) % {"body": match(ClassDef) % {"name": "@name"}}
...
def test_match_instances_same_args2():
    pattern = match(Module) % {
        # that owns in all its children, recursively (i.e: somewhere, at any depth)
        # a ClassDef instance
        "*": match(ClassDef)
